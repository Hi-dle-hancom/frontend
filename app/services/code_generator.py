...
class PythonCodeGenerator:
    ...
    async def generate_code(self, prompt: str, access_token: Optional[str] = None) -> str:
        ...
        if project_context == 'web_development':
            if any(keyword in prompt_lower for keyword in ['api', 'flask', 'django', 'fastapi']):
                return self._generate_web_api_code(user_preferences, prompt)
        elif project_context == 'data_science':
            if any(keyword in prompt_lower for keyword in ['데이터', 'data', '분석', 'analysis', 'pandas', 'numpy']):
                return self._generate_data_science_code(user_preferences, prompt)
        elif project_context == 'automation':
            if any(keyword in prompt_lower for keyword in ['자동화', 'automation', '스크립트', 'script']):
        ...
        elif any(keyword in prompt_lower for keyword in ['리스트', 'list', '배열', 'array']):
            return self._generate_list_operations(user_preferences)
        elif any(keyword in prompt_lower for keyword in ['딕셔너리', 'dict', 'dictionary']):
    ...
    async def _get_user_preferences(self, access_token: Optional[str]) -> Dict[str, Any]:
        ...
        from app.core.settings_mapper import map_db_to_preferences, get_default_user_preferences
    ...
    def _generate_function(self, preferences: Dict[str, Any]) -> str:
        ...
        if skill_level == 'beginner':
            return self._generate_beginner_function(comment_style, code_style, use_f_strings)
        elif skill_level in ['advanced', 'expert']:
            return self._generate_advanced_function(comment_style, code_style, error_handling, use_type_hints, use_f_strings)
        else:  # intermediate
            return self._generate_intermediate_function(comment_style, code_style, error_handling, use_type_hints, use_f_strings)
    
    def _generate_beginner_function(self, comment_style: str, code_style: str, use_f_strings: bool) -> str:
        """초급자용 함수 (자세한 설명 포함)"""
        if comment_style == 'educational':
            string_format = 'f"합계: {result}"' if use_f_strings else '"합계: " + str(result)'
...
print({string_format})'''
    ...
    def _generate_intermediate_function(self, comment_style: str, code_style: str, error_handling: str, use_type_hints: bool, use_f_strings: bool) -> str:
        """중급자용 함수"""
        type_annotation = ': List[Union[int, float]]' if use_type_hints else ''
        return_annotation = ' -> Union[int, float]' if use_type_hints else ''
        string_format = 'f"결과: {result}"' if use_f_strings else '"결과: " + str(result)'
        error_code = self._get_error_handling_code(error_handling) if error_handling != 'basic' else ''
...
print({string_format})'''
    ...
    def _generate_advanced_function(self, comment_style: str, code_style: str, error_handling: str, use_type_hints: bool, use_f_strings: bool) -> str:
        """고급자용 함수 (복잡한 로직 포함)"""
...
loaded_data = read_json_file("config.json")'''
    ...
    def _generate_api_code(self, preferences: Dict[str, Any], prompt: str) -> str:
        """API 요청 코드 생성"""
        skill_level = preferences.get('skill_level', 'intermediate')
        code_style = preferences.get('code_style', 'standard')
        error_handling = preferences.get('error_handling', 'basic')
...
if __name__ == "__main__":
    ...
    def _generate_data_science_code(self, preferences: Dict[str, Any], prompt: str) -> str:
        """데이터 사이언스 특화 코드 생성"""
        skill_level = preferences.get('skill_level', 'intermediate')
        code_style = preferences.get('code_style', 'standard')
        error_handling = preferences.get('error_handling', 'basic')
...
# stats = analyzer.exploratory_analysis()'''
    ...
    def _generate_automation_code(self, preferences: Dict[str, Any], prompt: str) -> str:
        """자동화 스크립팅 특화 코드 생성"""
        skill_level = preferences.get('skill_level', 'intermediate')
        error_handling = preferences.get('error_handling', 'basic')
        language_features = preferences.get('language_features', [])
