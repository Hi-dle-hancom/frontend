...
class ResponseParser:
    ...
    def parse_model_response(raw_response: str) -> Dict[str, str]:
        ...
        try:
            ...
            # 2. Python 코드 패턴 추출
            if not code_blocks:
                code_blocks = ResponseParser._extract_python_code_patterns(raw_response)
            
            # 3. 최적의 코드 블록 선택
            best_code = ResponseParser._select_best_code_block(code_blocks)
            
            # 4. 설명 추출
            explanation = ResponseParser._extract_explanation(raw_response, code_blocks)
    ...
    def _select_best_code_block(code_blocks: List[str]) -> str:
        ...
        for block in code_blocks:
            ...
            # Python 키워드 포함 여부
            python_keywords = ['def', 'class', 'import', 'for', 'if', 'while', 'try', 'except']
