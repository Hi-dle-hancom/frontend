...
class EnvironmentValidator:
    ...
    def _validate_single(
        self, key: str, rules: Dict[str, Any], is_production: bool
    ) -> Optional[ValidationResult]:
        """단일 환경 변수 검증"""
        value = os.getenv(key)

        # 필수 값 검증
        if rules.get("required", False) or (
            is_production and rules.get("required_in_production", False)
        ):
            if not value:
        ...
        if (
            is_production
            and "production_value" in rules
            and value.lower() != rules["production_value"].lower()
        ):
            return ValidationResult(
                key=key,
                is_valid=False,
                severity=rules["severity"],
                message=f"운영 환경에서 '{key}'는 '{
                    rules['production_value']}'이어야 합니다.",
                current_value=value,
            )
    ...
    def _validate_type(
        self, value: str, expected_type: str, rules: Dict[str, Any]
    ) -> bool:
        """타입 검증"""
        try:
    ...
    def get_validation_summary(
            self, results: List[ValidationResult]) -> Dict[str, Any]:
        """검증 결과 요약"""
        critical_count = sum(
    ...
    def log_validation_results(self, results: List[ValidationResult]) -> None:
        ...
        for result in results:
            ...
            log_level(f"[{result.severity.value.upper()}] {
...
def validate_environment_on_startup() -> bool:
    ...
    if summary["critical"] > 0:
        logger.critical(
            "🚨 Critical 환경 변수 오류로 인해 애플리케이션을 시작할 수 없습니다!"
        )
        return False
    ...
    if summary["errors"] > 0:
        logger.error(
            "⚠️ 환경 변수 오류가 있습니다. 일부 기능이 제대로 작동하지 않을 수 있습니다."
        )
...
def get_environment_health() -> Dict[str, Any]:
    ...
    return {
        "environment_variables": {
            ...
            "issues": summary,
            "details": [
                {"key": r.key, "severity": r.severity.value, "message": r.message}
                for r in results
            ],
        }
    }
