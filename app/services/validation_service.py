...
class SafeCodeExecutor:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.Attribute):
                    if isinstance(node.value,
                                  ast.Name) and node.value.id == "open":
    ...
    def execute_safely(
            self, code: str) -> Tuple[Optional[str], Optional[str], float]:
        """안전하게 코드 실행"""
...
class CodeValidationService:
    ...
    def validate_code(
            self,
            request: CodeValidationRequest) -> CodeValidationResponse:
        """
        코드를 검증합니다.
        ...
        try:
            ...
            # 1. 문법 분석
            syntax_issues = self._analyze_syntax(
            ...
            if (
                ...
                and not syntax_issues
            ):
                execution_result, execution_error, execution_time = (
                    self.executor.execute_safely(request.code)
                )
    ...
    def _save_validation_history(
        self, request: CodeValidationRequest, response: CodeValidationResponse
    ):
        """검증 이력을 저장합니다."""
        try:
            history_data = {
    ...
    def get_validation_stats(self) -> ValidationStats:
        ...
        try:
            ...
            valid_code_count = sum(
                1 for v in all_validations if v.get("is_valid", False)
            )
            invalid_code_count = total_validations - valid_code_count

            # 평균 검증 시간
            validation_times = [v.get("validation_time", 0)
                                for v in all_validations]
            average_validation_time = (
                sum(validation_times) /
                len(validation_times) if validation_times else 0)
