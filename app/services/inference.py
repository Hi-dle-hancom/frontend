...
from app.services.cache_service import cache_exists, cache_get, cache_set
...
from app.services.response_parser import ErrorDetector, ResponseParser
...
class OptimizedAIModelService:
        ...
        with profiler.profile_function("ai_prediction"):
            try:
                ...
                cache_key = self._generate_personalized_cache_key(
                    prompt, context, language, access_token
                )
                ...
                generated_code = await self.code_generator.generate_code(
                    processed_prompt, access_token
                )

                # 결과 캐싱 (개인화된 경우 짧은 TTL)
                ttl = (
                    1800 if access_token else 3600
                )  # 개인화된 캐시는 30분, 일반 캐시는 1시간
                cache_set(cache_key, generated_code, ttl=ttl)

                logger.info(
                    f"개인화된 Python 코드 생성 완료 - 길이: {len(generated_code)}, 개인화: {bool(access_token)}"
                )
    ...
    async def predict_and_parse(
        ...
        try:
            # 개인화된 캐시 키 생성 (파싱 결과용)
            cache_key = self._generate_personalized_cache_key(
                f"parsed:{prompt}", context, language, access_token, user_profile)
            ...
            raw_response = await self._generate_raw_model_response(
                prompt, context, language, access_token, user_profile
            )
            ...
            # 개인화된 결과 캐싱 (짧은 TTL)
            ttl = (
                900 if access_token or user_profile else 1800
            )  # 개인화: 15분, 일반: 30분
    ...
    async def _generate_raw_model_response(
        self,
        prompt: str,
        ...
        cache_key = self._generate_personalized_cache_key(
            f"raw:{prompt}", context, language, access_token, user_profile
        )
...
위 코드를 사용하여 원하는 기능을 구현할 수 있습니다."""
    ...
    def complete_code(
        self, prefix: str, language: str = "python", max_suggestions: int = 3
    ) -> List[str]:
        """코드 자동 완성 제안을 생성합니다."""
        ...
        cache_key = self._generate_cache_key(
            f"completion:{prefix}", None, language)
        ...
        # 새로운 자동완성 생성
        completions = self.code_generator.generate_completion(
            prefix, max_suggestions)
    ...
    def _generate_personalized_cache_key(
        ...
        if access_token:
            ...
            token_hash = hashlib.sha256(token_data.encode(
                "utf-8")).hexdigest()[:16]  # 보안 + 성능 균형
        ...
        if user_profile:
            ...
            try:
                ...
                logger.warning(
                    f"사용자 프로필 캐시 키 생성 중 오류 (안전한 기본값 사용): {e}"
                )
    ...
    def _generate_personalized_explanation(
            self, prompt: str, user_profile=None) -> str:
