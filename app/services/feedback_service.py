...
class FeedbackService:
    ...
    def get_feedback_stats(self) -> FeedbackStats:
        ...
        try:
            ...
            like_count = sum(
                1 for f in all_feedback if f.get("feedback_type") == "like"
            )
            dislike_count = sum(
                1 for f in all_feedback if f.get("feedback_type") == "dislike"
            )
            comment_count = sum(1 for f in all_feedback if f.get("comment"))

            # 평균 별점 계산
            ratings = [f.get("rating")
                       for f in all_feedback if f.get("rating") is not None]
    ...
    def get_feedback_by_session(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            all_feedback = self._load_feedback_data()
            session_feedback = [
                f for f in all_feedback if f.get("session_id") == session_id
            ]
    ...
    def get_recent_feedback(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            sorted_feedback = sorted(
                all_feedback, key=lambda x: x.get(
    ...
    def delete_feedback(self, feedback_id: str) -> bool:
        ...
        try:
            ...
            # 해당 ID의 피드백 제거
            filtered_feedback = [
                f for f in all_feedback if f.get("feedback_id") != feedback_id
            ]
