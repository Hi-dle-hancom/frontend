...
class StructuredLogger:
    ...
    def log_ai_generation(
        ...
        entry.metadata = {
            ...
            "cost": cost,
            "cache_hit": cache_hit,
            "efficiency_ratio": response_length / prompt_length if prompt_length > 0 else 0
        }
    ...
    def log_system_event(
        ...
        """시스템 이벤트 로그"""
        level = LogLevel.ERROR if status in ["failed", "error"] else LogLevel.INFO
        
        entry = self._create_base_entry(
            level,
            LogCategory.SYSTEM_EVENT,
            f"시스템 이벤트: {event} - {status}"
        )
...
def log_api_response(method: str, endpoint: str, status_code: int, response_time_ms: float, **kwargs):
    """API 응답 로그 편의 함수"""
    structured_logger.log_api_response(method, endpoint, status_code, response_time_ms, **kwargs)

def log_ai_generation(model_name: str, prompt_length: int, response_length: int, generation_time_ms: float, **kwargs):
    """AI 생성 로그 편의 함수"""
    structured_logger.log_ai_generation(model_name, prompt_length, response_length, generation_time_ms, **kwargs)
