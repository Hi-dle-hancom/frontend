...
class SettingsMapper:
    ...
    OPTION_ID_MAPPING = {
        ...
        # 에러 처리 선호도 (ID: 25-27)
        SettingCategory.ERROR_HANDLING: {
            25: "basic",
            26: "detailed",
            27: "robust"
        }
    ...
    def _build_reverse_mapping(self):
        ...
        for category, id_value_map in self.OPTION_ID_MAPPING.items():
            self.VALUE_TO_ID_MAPPING[category] = {v: k for k, v in id_value_map.items()}
    
    def map_db_settings_to_preferences(self, db_settings: List[Dict[str, Any]]) -> Dict[str, Any]:
        """DB 설정을 사용자 선호도로 변환"""
    ...
    def map_user_profile_to_preferences(self, user_profile, base_preferences: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            safe_mappings = {
                ...
                'errorHandlingPreference': ('error_handling', self._validate_error_handling),
                'preferredLanguageFeatures': ('language_features', self._validate_language_features)
            }
    ...
    def map_onboarding_data_to_option_ids(self, onboarding_data: Dict[str, Any]) -> List[int]:
        ...
        try:
            ...
            skill_id = self._get_id_by_value(SettingCategory.PYTHON_SKILL, skill_level)
            if skill_id:
                option_ids.append(skill_id)
            
            # 코드 출력 구조
            output_structure = onboarding_data.get('outputStructure', 'standard')
            output_id = self._get_id_by_value(SettingCategory.CODE_OUTPUT, output_structure)
            if output_id:
                option_ids.append(output_id)
            
            # 설명 스타일
            explanation_style = onboarding_data.get('explanationStyle', 'standard')
            explanation_id = self._get_id_by_value(SettingCategory.EXPLANATION, explanation_style)
            if explanation_id:
                option_ids.append(explanation_id)
            
            # 프로젝트 컨텍스트
            project_context = onboarding_data.get('projectContext', 'general_purpose')
            context_id = self._get_id_by_value(SettingCategory.PROJECT_CONTEXT, project_context)
            if context_id:
                option_ids.append(context_id)
            
            # 주석 트리거 모드
            trigger_mode = onboarding_data.get('commentTriggerMode', 'confirm')
            trigger_id = self._get_id_by_value(SettingCategory.COMMENT_TRIGGER, trigger_mode)
            if trigger_id:
            ...
            if isinstance(language_features, list):
                for feature in language_features:
                    feature_id = self._get_id_by_value(SettingCategory.LANGUAGE_FEATURES, feature)
                    if feature_id:
                        option_ids.append(feature_id)
            
            # 에러 처리 선호도
            error_handling = onboarding_data.get('errorHandling', 'basic')
            error_id = self._get_id_by_value(SettingCategory.ERROR_HANDLING, error_handling)
            if error_id:
    ...
    def _get_id_by_value(self, category: SettingCategory, value: str) -> Optional[int]:
        """카테고리와 값으로 옵션 ID 조회"""
        try:
    ...
    def _validate_project_context(self, value) -> Optional[str]:
        valid_contexts = ['web_development', 'data_science', 'automation', 'general_purpose']
    ...
    def _validate_language_features(self, value) -> Optional[List[str]]:
        if not isinstance(value, list):
            return None
        valid_features = ['type_hints', 'dataclasses', 'async_await', 'f_strings']
...
def map_profile_to_preferences(user_profile, base_preferences: Dict[str, Any] = None) -> Dict[str, Any]:
    """userProfile → 선호도 변환 (편의 함수)"""
    return settings_mapper.map_user_profile_to_preferences(user_profile, base_preferences)
