...
class ConversationEntry(BaseModel):
    ...
    language: Optional[str] = Field(
        None, description="프로그래밍 언어", max_length=50)
    code_snippet: Optional[str] = Field(
        None, description="관련 코드 스니펫", max_length=5000
    )
    ...
    confidence_score: Optional[float] = Field(
        None, description="신뢰도 점수 (0-1)", ge=0, le=1
    )
...
class ConversationSession(BaseModel):
    ...
    session_title: Optional[str] = Field(
        None, description="세션 제목", max_length=200)
    ...
    project_name: Optional[str] = Field(
        None, description="프로젝트명", max_length=100)
...
class HistoryCreateRequest(BaseModel):
    ...
    confidence_score: Optional[float] = Field(
        None, description="신뢰도 점수", ge=0, le=1
    )
