...
class FeedbackRequest(BaseModel):
    ...
    question_id: str = Field(..., description="질문 ID", min_length=1, max_length=100)
    response_id: str = Field(..., description="응답 ID", min_length=1, max_length=100)
    
    # 피드백 내용
    rating: Optional[int] = Field(None, description="별점 (1-5)", ge=1, le=5)
    comment: Optional[str] = Field(None, description="텍스트 코멘트", max_length=1000)
