...
class ValidationIssue(BaseModel):
    ...
    suggestion: Optional[str] = Field(None, description="수정 제안", max_length=300)
...
class CodeValidationRequest(BaseModel):
    """코드 검증 요청 모델"""
    code: str = Field(..., description="검증할 코드", min_length=1, max_length=10000)
    language: str = Field("python", description="프로그래밍 언어", pattern="^(python|javascript|typescript|java|cpp|csharp)$")
    ...
    context: Optional[str] = Field(None, description="코드 컨텍스트", max_length=2000)
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "most_common_issues": ["SyntaxError", "NameError", "IndentationError"],
                "language_distribution": {"python": 480, "javascript": 20}
            }
        }
