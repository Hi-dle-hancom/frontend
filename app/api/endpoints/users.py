...
async def login_user(user_data: UserLoginRequest):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise HTTPException(
                status_code=response.status_code, detail="사용자 인증에 실패했습니다."
            )

    except httpx.TimeoutException:
        log_system_event(
            "DB Module 연결 타임아웃", "failed", details={
                "url": settings.DB_MODULE_URL})
        raise HTTPException(
...
async def generate_api_key(
    ...
    try:
        ...
        log_system_event("API 키 발급 오류", "error", details={"error": str(e)})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.post("/login-activity", response_model=Dict[str, Any])
async def login_user_activity_based(user_data: UserLoginRequest):
    """
    활동기반 인증 로그인/등록
    이메일과 사용자명을 받아 활동기반 토큰을 반환합니다.
    """
    try:
        log_user_action(
            "활동기반 로그인 요청",
        ...
        # DB Module 활동기반 인증에 로그인 요청
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{settings.DB_MODULE_URL}/api/v1/activity-auth/login",
                json={
                    "email": user_data.email,
                    "password": "hapa_default_password",  # 패스워드 없는 시스템이므로 더미값
                },
                timeout=settings.DB_MODULE_TIMEOUT,
            )
        ...
        if response.status_code == 200:
            auth_data = response.json()
            log_user_action(
                "활동기반 로그인 성공", user_data.email, details={
                    "session_id": auth_data.get(
                        "token_info", {}).get("session_id")}, )
            return {
                "message": "활동기반 로그인 성공",
                "user": auth_data.get("user", {}),
                "token_info": auth_data.get("token_info", {}),
                "activity_settings": auth_data.get("activity_settings", {}),
            }
        else:
            log_user_action(
                "활동기반 로그인 실패",
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="활동기반 인증 서비스가 일시적으로 이용불가합니다.",
        )
    except Exception as e:
        log_system_event("활동기반 로그인 오류", "error", details={"error": str(e)})
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="활동기반 로그인 처리 중 오류가 발생했습니다.",
        )
...
async def update_my_settings(
    settings_data: UserSettingsRequest, authorization: str = Header(...)
):
    """
    사용자 개인화 설정 업데이트
    """
    try:
        ...
        success = await user_service.update_user_settings(
            access_token=access_token, option_ids=settings_data.option_ids
        )
...
async def save_user_profile(
    profile_request: UserProfileRequest, authorization: str = Depends(security)
):
    """
    온보딩 사용자 프로필 저장
    ...
    try:
        ...
        log_user_action(
            ...
            details={
                "profile_data_keys": (
                    list(profile_request.profile_data.keys())
                    if profile_request.profile_data
                    else []
                ),
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{settings.DB_MODULE_URL}/users/me/settings",
                json={"option_ids": profile_request.settings_mapping},
                headers={
        ...
        if response.status_code == 204:
            log_user_action(
                "프로필 저장 성공", "unknown", details={
                    "settings_count": len(
                        profile_request.settings_mapping)}, )
            ...
            raise HTTPException(
                status_code=response.status_code, detail="프로필 저장에 실패했습니다."
            )
