...
class PromptExtractor {
     ...
     * 선택된 텍스트 분석 (주석 인식 포함)
     */
    static analyzeSelectedText(text, language) {
        ...
        if (isComment) {
            ...
            const enhancedPrompt = this.enhanceCommentPrompt(trimmedText, intent, language);
            return {
                isComment: true,
                intent,
                enhancedPrompt,
            ...
            const enhancedPrompt = this.enhanceCodePrompt(trimmedText, codeIntent, language);
            return {
                isComment: false,
                intent: codeIntent,
                enhancedPrompt,
     ...
     * 주석 의도 분석
     */
    static analyzeCommentIntent(comment) {
        // 주석 기호 제거
     ...
     * 코드 의도 분석
     */
    static analyzeCodeIntent(code, language) {
     ...
     * 주석 프롬프트 개선
     */
    static enhanceCommentPrompt(comment, intent, language) {
        const cleanComment = comment
            .replace(/^\s*#\s*/, "")
            .replace(/^\s*\/\/\s*/, "")
            .trim();
     ...
     * 코드 프롬프트 개선
     */
    static enhanceCodePrompt(code, intent, language) {
