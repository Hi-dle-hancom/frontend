...
class CodeCompletionProvider {
    ...
    async generateCode(request) {
        ...
        const safeRequest = {
            ...
            model_type: request.model_type || apiClient_1.VLLMModelType.CODE_GENERATION,
        ...
        try {
            ...
            const response = await axios_1.default.post(`${this.baseURL}/code/generate`, safeRequest, {
                headers: {
    ...
    async generateCompletion(prompt, language = "python") {
        const request = {
            prompt,
            model_type: apiClient_1.VLLMModelType.CODE_COMPLETION,
            language,
    ...
    async completeCode(request) {
        ...
        try {
            const response = await axios_1.default.post(`${this.baseURL}/code/complete`, {
            ...
            try {
                ...
                const fallbackResponse = await this.generateAutoComplete(request.prefix, request.language);
                // 응답 형식 변환
                return {
                    completions: [
                        {
     ...
     */
    async generateInlineCompletion(prefix, suffix = "", language = "python") {
     ...
     * 🧠 컨텍스트 기반 스마트 완성
     */
    async generateSmartCompletion(prefix, fileContext, language = "python") {
...
`;
     ...
     * 🎨 함수 시그니처 완성
     */
    async generateFunctionCompletion(functionName, context = "", language = "python") {
        const prompt = `
...
`;
     ...
     * 🔧 클래스 메서드 완성
     */
    async generateMethodCompletion(className, methodName, classContext = "", language = "python") {
        const prompt = `
클래스 컨텍스트:
${classContext}
