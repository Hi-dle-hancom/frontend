...
exports.StreamingCodeGenerator = exports.APIError = void 0;
...
class StreamingCodeGenerator {
    ...
    validateAndSanitizeRequest(request) {
        ...
        const safeRequest = {
            ...
            model_type: request.model_type || types_1.VLLMModelType.CODE_GENERATION,
    ...
    async generateCodeStream(request, onChunk, onComplete, onError) {
        ...
        try {
            ...
            const apiError = error instanceof APIError ? error : new APIError("요청 검증 실패", 400);
            onError?.(apiError);
            return;
        }
        // 환경별 조건부 로깅 - 요청 시작
        ...
        try {
            ...
            let jwtToken = config.get("auth.accessToken");
            // JWT 토큰 만료 체크
            ...
            const response = await axios_1.default.post(`${this.baseURL}/code/generate/stream?enhanced=true`, // Enhanced 모드 활성화
            safeRequest, {
                headers,
                responseType: "stream",
                    ...
                    for (const line of lines) {
                        ...
                        try {
                            ...
                            if (rawChunk.text !== undefined) {
                                // 환경별 조건부 로깅 - 청크 변환 상세
                                if (DEBUG_MODE) {
                                    console.log(`📦 백엔드 청크 변환: "${rawChunk.text}" → StreamingChunk`);
                                }
                                parsedChunk = {
            ...
            if (axios_1.default.isCancel(error)) {
                ...
                apiError = new APIError("요청이 취소되었습니다.", 408, "REQUEST_CANCELLED");
                // 취소된 요청에 대한 정리 작업
                try {
            ...
            if (error.name === "AbortError") {
                ...
                apiError = new APIError("요청이 중단되었습니다.", 408, "REQUEST_ABORTED");
                onError?.(apiError);
                return;
            }
            // 네트워크 오류 처리
            if (error.code === "ECONNABORTED" || error.message.includes("timeout")) {
                ...
                apiError = new APIError("응답 시간이 초과되었습니다. 더 간단한 요청으로 다시 시도해주세요.", 408, "TIMEOUT_ERROR");
            }
            else if (error.code === "ECONNREFUSED" || error.code === "ENOTFOUND") {
                console.error("🌐 네트워크 연결 실패");
                apiError = new APIError("서버에 연결할 수 없습니다. 네트워크 상태를 확인해주세요.", 503, "NETWORK_ERROR");
            }
            else if (error.code === "ECONNRESET") {
                console.error("🔌 연결이 재설정됨");
                apiError = new APIError("네트워크 연결이 재설정되었습니다. 다시 시도해주세요.", 503, "CONNECTION_RESET");
            }
                ...
                apiError = new APIError(error.message || "알 수 없는 오류가 발생했습니다", 500, "UNKNOWN_ERROR");
            }
    ...
    async generateCodeStreaming(prompt, codeContext, callbacks) {
        ...
        return this.generateCodeStream(request, callbacks.onChunk || (() => { }), callbacks.onComplete, callbacks.onError);
    }
    /**
     * 🔧 버그 수정용 스트리밍 코드 생성
     */
    async generateBugFixStream(prompt, codeContext, callbacks) {
        const request = {
            prompt,
            context: codeContext,
            model_type: types_1.VLLMModelType.BUG_FIX,
     ...
     * 📝 코드 설명용 스트리밍
     */
    async generateExplanationStream(code, callbacks) {
        const request = {
            prompt: `다음 코드를 상세히 설명해주세요:\n\n${code}`,
            model_type: types_1.VLLMModelType.CODE_EXPLANATION,
     ...
     * 🧪 단위 테스트 생성용 스트리밍
     */
    async generateTestStream(code, callbacks) {
        const request = {
            prompt: `다음 코드에 대한 단위 테스트를 생성해주세요:\n\n${code}`,
            model_type: types_1.VLLMModelType.UNIT_TEST_GENERATION,
     ...
     * ⚡ 코드 최적화용 스트리밍
     */
    async generateOptimizationStream(code, callbacks) {
        const request = {
            prompt: `다음 코드를 성능과 가독성 측면에서 최적화해주세요:\n\n${code}`,
            model_type: types_1.VLLMModelType.CODE_OPTIMIZATION,
     ...
     * 🔍 코드 리뷰용 스트리밍
     */
    async generateReviewStream(code, callbacks) {
        const request = {
            prompt: `다음 코드를 검토하고 개선사항을 제안해주세요:\n\n${code}`,
            model_type: types_1.VLLMModelType.CODE_REVIEW,
            explanation_detail: "comprehensive",
            code_style: "clean",
        };
        return this.generateCodeStream(request, callbacks.onChunk || (() => { }), callbacks.onComplete, callbacks.onError);
