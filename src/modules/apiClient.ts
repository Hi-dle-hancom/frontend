...
// 내부 모듈 import
import { StreamingCodeGenerator } from "./StreamingCodeGenerator";
import {
...
export class HAPAAPIClient {
  ...
  constructor(apiKey: string = "") {
    ...
    this.streamingGenerator = new StreamingCodeGenerator(this.configService);
  ...
  async generateCodeStream(
    ...
    return this.streamingGenerator.generateCodeStream(
      request,
      onChunk,
      onComplete,
      onError
   ...
   */
  private async legacyGenerateCodeStream(
    request: CodeGenerationRequest,
    onChunk: (chunk: StreamingChunk) => void,
    onComplete?: () => void,
    onError?: (error: APIError) => void
  ...
  async generateCode(
    ...
    try {
      ...
      const apiRequest: CodeGenerationRequest = {
        ...
        model_type: request.model_type || VLLMModelType.CODE_GENERATION, // Backend가 기대하는 model_type 사용
      ...
      if (axiosError.response) {
        ...
        switch (status) {
          case 400:
            return {
              message:
                data?.message || "잘못된 요청입니다. 입력 내용을 확인해주세요.",
              status: status,
          ...
          case 500:
            return {
              message:
                "서버 내부 오류입니다. 계속 문제가 발생하면 관리자에게 문의하세요.",
              status: status,
  ...
  async generateCodeStreaming(
    ...
    return this.streamingGenerator.generateCodeStreaming(
      prompt,
      codeContext,
      callbacks
    );
...
export function initializeAPIClient(): void {
  ...
  apiClient.updateConfig(apiConfig.apiKey, apiConfig.baseURL);
  ...
  if (!apiConfig.apiKey) {
    ...
    apiClient.updateConfig(demoKey, apiConfig.baseURL);
  }
