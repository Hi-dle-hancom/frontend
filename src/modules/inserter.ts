...
export interface InsertOptions {
  position?: "cursor" | "selection" | "end" | "newFile";
  language?: string;
  filename?: string;
  replace?: boolean;
}
...
export class CodeInserter {
  ...
  static async insertAtCursor(
    ...
    try {
      const success = await editor.edit((editBuilder) => {
        ...
        // 코드 포맷팅 (들여쓰기 맞추기)
        const formattedCode = this.formatCodeForInsertion(
          code,
          editor,
          position
        );
  ...
  static async replaceSelection(
    ...
    try {
      const success = await editor.edit((editBuilder) => {
        ...
        // 코드 포맷팅
        const formattedCode = this.formatCodeWithIndent(
          code,
          indentLevel,
  ...
  static async appendToFile(
    ...
    try {
      ...
      const success = await editor.edit((editBuilder) => {
        ...
        const formattedCode =
          prefix + this.formatCodeForInsertion(code, editor, position);
  ...
  static async createNewFile(
    ...
    try {
      ...
      const extension =
        extensions[language as keyof typeof extensions] || ".txt";
  ...
  static async insertTest(
    ...
    try {
      ...
      try {
        ...
        // 테스트 파일이 없으면 새로 생성
        const testFileName =
          testFilePath.split("/").pop() || "test_generated.py";
        return this.createNewFile(testCode, {
          language: "python",
   ...
   * 코드를 삽입 위치에 맞게 포맷팅
   */
  private static formatCodeForInsertion(
    code: string,
   ...
   * 들여쓰기 레벨 감지
   */
  private static detectIndentLevel(text: string): number {
   ...
   * 지정된 들여쓰기 레벨로 코드 포맷팅
   */
  private static formatCodeWithIndent(
    code: string,
    indentLevel: number,
    tabSize: number
  ): string {
   ...
   * 테스트 파일 경로 생성
   */
  private static generateTestFilePath(originalPath: string): string {
