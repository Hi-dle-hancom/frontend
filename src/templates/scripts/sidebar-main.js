...
const streamingManager = {
    ...
    switch (newState) {
      ...
      case StreamingState.ACTIVE:
        if (previousState === StreamingState.IDLE) {
          console.warn(
            `⚠️ 예상치 못한 전환: ${previousState} → ${newState} (자동 복구)`
          );
        }
        break;
  ...
  async updateStreamingDisplay(content) {
    ...
    if (
      typeof messageHandler !== "undefined" &&
      messageHandler.updateStreamingDisplay
    ) {
      ...
      const responseElement = document.getElementById("response-content");
      if (responseElement) {
        responseElement.innerHTML = `<pre><code>${this.escapeHtml(
          streamingBuffer
        )}</code></pre>`;
      }
    }
...
const testSuite = {
  ...
  testStateTransitions() {
    ...
    for (const testCase of testCases) {
      ...
      for (const transition of testCase.transitions) {
        // 상태 설정
        streamingManager.currentState =
          StreamingState[transition.from.toUpperCase()];

        // 전환 시도
        const result = streamingManager.setState(
          StreamingState[transition.to.toUpperCase()],
          { test: true }
        );
  ...
  async testUIRecovery() {
    ...
    try {
      ...
      if (responseElement) {
        responseElement.innerHTML =
          '<div class="error-message">테스트 에러 메시지</div>';
      }
      ...
      // 3. 복구 결과 검증
      const hasError =
        responseElement && responseElement.innerHTML.includes("error-message");
