 ...
 * 요청 복잡도 분석기
 */
export class RequestComplexityAnalyzer {
  ...
  analyzeComplexity(context: RequestContext): ComplexityLevel {
    const text = `${context.prompt} ${
      context.hasContext ? "with_context" : ""
    }`.toLowerCase();
 ...
 * 적응형 성능 제한기
 */
export class AdaptivePerformanceLimiter {
  private analyzer = new RequestComplexityAnalyzer();
  ...
  private baseConfigs: Record<ComplexityLevel, PerformanceLimits> = {
  ...
  private currentLimits: PerformanceLimits | null = null;
  private currentComplexity: ComplexityLevel | null = null;
   ...
   */
  calculateLimits(context: RequestContext): PerformanceLimits {
  ...
  private adjustBasedOnHistory(
    ...
    limits.maxProcessingTime = Math.round(
      limits.maxProcessingTime * timeMultiplier
    );
   ...
   * 시스템 리소스 기반 조정
   */
  private adjustBasedOnSystemLoad(limits: PerformanceLimits): void {
    // 메모리 사용량 확인 (근사치)
   ...
   * 사용자 패턴 기반 조정
   */
  private adjustBasedOnUserPattern(
    limits: PerformanceLimits,
   ...
   * 현재 성능 제한 반환
   */
  getCurrentLimits(): PerformanceLimits | null {
   ...
   * 현재 복잡도 반환
   */
  getCurrentComplexity(): ComplexityLevel | null {
  ...
  getDiagnostics(): {
    ...
    currentLimits: PerformanceLimits | null;
    currentComplexity: ComplexityLevel | null;
    historySize: number;
    ...
    // 복잡도별 최근 성능 분석
    const recentPerformance = Object.values(ComplexityLevel).map(
      (complexity) => {
        ...
        const avgResponseTime =
          complexityHistory.reduce((sum, h) => sum + h.responseTime, 0) /
          complexityHistory.length;
        const successRate =
          complexityHistory.filter((h) => h.success).length /
          complexityHistory.length;
  ...
  checkPerformanceWarnings(): string[] {
    ...
    if (this.metrics.averageResponseTime > limits.maxProcessingTime * 0.8) {
      warnings.push(
        `응답 시간이 길어지고 있습니다 (평균: ${Math.round(
          this.metrics.averageResponseTime
        )}ms)`
      );
    }
    ...
    if (this.metrics.errorRate > 0.2) {
      warnings.push(
        `오류율이 높습니다 (${Math.round(this.metrics.errorRate * 100)}%)`
      );
    }
  ...
  getOptimizationSuggestions(): string[] {
    ...
    if (
      ...
      suggestions.push(
        "프롬프트를 더 구체적으로 작성하여 응답 시간을 단축할 수 있습니다."
      );
...
// 싱글톤 인스턴스 생성 및 내보내기
export const adaptivePerformanceLimiter = new AdaptivePerformanceLimiter();
