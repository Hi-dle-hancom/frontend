 ...
 */
class SidebarDashboardViewProvider {
    _extensionUri;
    _view;
    triggerDetector;
    ...
    async handleTriggerEvent(event) {
        ...
        try {
            ...
            const request = {
                ...
                requestType: event.action === "analyze"
                    ? "analyze"
                    : event.action === "test"
                        ? "test"
                        : "generate",
            ...
            // 실제 AI API 호출
            const response = await apiClient_1.apiClient.generate(request);
            // 웹뷰에 응답 전송
            this._view.webview.postMessage({
		...
		<body>
		  <div class="container">
			...
			<div class="setting-item">
			  ...
			  >
				<option value="light" ${config.get("theme") === "light" ? "selected" : ""}>라이트</option>
				<option value="dark" ${config.get("theme") === "dark" ? "selected" : ""}>다크</option>
				<option value="system" ${config.get("theme") === "system" ? "selected" : ""}>시스템</option>
			  </select>
			</div>
		  </div>
		  <script>
...
function activate(context) {
    // 모듈 인스턴스들 생성
    const triggerDetector = new triggerDetector_1.TriggerDetector();
    console.log("한컴 AI Extension이 활성화되었습니다!");
    // 웹뷰 프로바이더 등록
    const sidebarDashboardProvider = new SidebarDashboardViewProvider(context.extensionUri);
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider("hapa-dashboard", sidebarDashboardProvider));
    // 명령어 등록
    context.subscriptions.push(vscode.commands.registerCommand("hapa.start", () => {
    ...
    vscode.commands.registerCommand("hapa.explain", () => {
        ...
        await inserter_1.CodeInserter.smartInsert(code);
    }), 
    // 웹사이트로 이동하는 명령어 추가
    ...
    vscode.commands.registerCommand("hapa.showGuide", () => {
        const panel = vscode.window.createWebviewPanel("hapaGuide", "HAPA 가이드", vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        const panel = vscode.window.createWebviewPanel("hapaSettings", "HAPA 설정", vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [context.extensionUri],
        });
