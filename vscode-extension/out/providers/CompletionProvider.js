...
exports.HAPAInlineCompletionProvider = exports.HAPACompletionProvider = void 0;
const vscode = __importStar(require("vscode"));
const apiClient_1 = require("../modules/apiClient");
/**
 * HAPA AI 기반 자동 완성 제공자
 */
class HAPACompletionProvider {
    triggerCharacters = [".", "(", "[", '"', "'", " "];
    cache = new Map();
    cacheTimeout = 5000; // 5초 캐시
    constructor() {
        // 캐시 정리를 위한 타이머 설정
        setInterval(() => {
            this.cache.clear();
    ...
    async provideCompletionItems(document, position, token, context) {
        ...
        try {
            ...
            const confidenceThreshold = config.get("autoComplete.confidenceThreshold", 0.3);
            const enableContextAnalysis = config.get("autoComplete.enableContextAnalysis", true);
            const completionContext = this.extractCompletionContext(document, position);
            // 캐시 확인
            ...
            if (response.status === "success" && response.completions) {
                // 신뢰도 필터링
                const filteredCompletions = response.completions.filter((completion) => completion.confidence >= confidenceThreshold);
                // 최대 개수 제한
                const limitedCompletions = filteredCompletions.slice(0, maxSuggestions);
                const items = this.createCompletionItems(limitedCompletions, completionContext);
                // 캐시 저장
     ...
     * 현재 스코프 찾기 (함수, 클래스 등)
     */
    findCurrentScope(document, position) {
     ...
     * 완성 아이템 생성
     */
    createCompletionItems(completions, context) {
        return completions.map((completion, index) => {
            const item = new vscode.CompletionItem(completion.label || completion.code, this.getCompletionItemKind(completion.category));
            // 기본 정보 설정
     ...
     * 완성 아이템 종류 결정
     */
    getCompletionItemKind(category) {
        switch (category) {
            case "function":
    ...
    generateDetailText(completion) {
        ...
        if (completion.complexity) {
            ...
            parts.push(`복잡도: ${complexityMap[completion.complexity] || completion.complexity}`);
        }
        if (completion.performance_impact) {
            ...
            parts.push(`성능 영향: ${impactMap[completion.performance_impact] ||
     ...
     * 문서화 생성
     */
    generateDocumentation(completion) {
        ...
        if (completion.explanation) {
            markdown.appendMarkdown(`**AI 추천 이유:** ${completion.explanation}\n\n`);
        }
        // 예상 결과
        if (completion.expected_result) {
            markdown.appendMarkdown(`**예상 결과:** \`${completion.expected_result}\`\n\n`);
        }
        // 코드 예시
        if (completion.examples && completion.examples.length > 0) {
            markdown.appendMarkdown(`**사용 예시:**\n`);
            completion.examples.forEach((example, index) => {
        ...
        if (completion.related_concepts && completion.related_concepts.length > 0) {
            markdown.appendMarkdown(`**관련 개념:** ${completion.related_concepts.join(", ")}\n\n`);
        }
        // 문서 링크
        if (completion.documentation_url) {
            markdown.appendMarkdown(`[📚 공식 문서 보기](${completion.documentation_url})`);
        }
        return markdown;
    }
    /**
     * 정렬 텍스트 생성 (신뢰도 기반)
     */
    generateSortText(confidence, index) {
        // 신뢰도가 높을수록 앞에 표시
     ...
     * 트리거 문자 확인
     */
    shouldTriggerCompletion(document, position) {
        const line = document.lineAt(position);
        const char = line.text.charAt(position.character - 1);
        return this.triggerCharacters.includes(char);
    }
    async generateCodeResponse(userQuestion, codeContext) {
        try {
            const response = await apiClient_1.apiClient.generatePersonalizedCode({
...
exports.HAPACompletionProvider = HAPACompletionProvider;
/**
 * 인라인 완성 제공자 (GitHub Copilot 스타일)
 */
class HAPAInlineCompletionProvider {
    lastRequest = null;
    ...
    async provideInlineCompletionItems(document, position, context, token) {
        ...
        try {
            ...
            if (response.status === "success" &&
                ...
                const insertText = this.extractInsertText(prefix, completion.insertion_text || completion.code);
                if (insertText.trim()) {
     ...
     * 삽입할 텍스트 추출 (중복 제거)
     */
    extractInsertText(prefix, fullCompletion) {
        ...
        let overlapIndex = 0;
        for (let i = 0; i < Math.min(prefixWords.length, completionWords.length); i++) {
...
exports.HAPAInlineCompletionProvider = HAPAInlineCompletionProvider;
