...
위 내용에 이어서 완성해주세요.`;
        ...
        try {
            await apiClient_1.apiClient.generateCodeStreaming(fullPrompt, codeContext, callbacks);
        }
        catch (error) {
     ...
     * 스트리밍 코드 생성 처리
     */
    async handleStreamingCodeGeneration(question) {
        if (!this._view?.webview) {
            return;
        }
        // 현재 활성 편집기의 컨텍스트 가져오기
    ...
    getExpandedViewHtml(webview) {
        ...
        return SidebarHtmlGenerator_1.SidebarHtmlGenerator.generateSidebarHtml()
            .replace("<title>HAPA - Sidebar Dashboard</title>", "<title>HAPA - Expanded View</title>")
            .replace("const vscode = acquireVsCodeApi();", `const vscode = acquireVsCodeApi();
      // 확장된 뷰 플래그 설정
     ...
     * 확장된 뷰의 메시지 처리
     */
    handleExpandedViewMessage(message, panel) {
        switch (message.command) {
            case "generateCodeStreaming":
