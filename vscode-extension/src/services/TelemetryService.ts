...
export class TelemetryService {
  ...
  private userId: string; // 익명화된 고유 ID
  ...
  private isDataCollectionConsented = false;
  ...
  constructor() {
    ...
    this.dataStorePath = require("path").join(
   ...
   * 사용자 동의 확인 및 활성화
   */
  private async checkConsentAndEnable(): Promise<void> {
    ...
    if (telemetryEnabled && !this.isDataCollectionConsented) {
      const consent = await vscode.window.showInformationMessage(
        "📊 HAPA 개선을 위해 익명화된 사용 통계를 수집할 수 있나요?\n" +
          "수집되는 정보: 기능 사용 빈도, 성능 지표, 오류 통계\n" +
          "개인 정보나 코드는 수집되지 않습니다.",
        "동의",
        "거부",
        "자세히 보기"
      );
      ...
      switch (consent) {
        ...
        case "자세히 보기":
          vscode.env.openExternal(
            vscode.Uri.parse("https://hapa-docs.com/privacy-policy")
          );
          break;
      }
    } else {
    ...
    console.log(
      `📊 텔레메트리 상태: ${this.isEnabled ? "활성화" : "비활성화"}`
    );
  }
  ...
  generateUsageReport(): string {
    ...
    const totalFeatureUsage = Array.from(
      this.userBehavior.featureUsage.values()
...
${Array.from(this.usageMetrics.features.entries())
  ...
  .map(
    ([feature, stats]) =>
      `• ${feature}: ${(stats.successRate * 100).toFixed(1)}% (${
        stats.count
      }회 사용)`
  )
  .join("\n")}
...
${this.generateImprovementSuggestions()}
  ...
  private generateImprovementSuggestions(): string {
    ...
    if (lowSuccessFeatures.length > 0) {
      suggestions.push(
        `- 성공률이 낮은 기능들의 UX 개선 필요: ${lowSuccessFeatures
          .map(([name]) => name)
          .join(", ")}`
      );
    }
    ...
    if (underusedFeatures.length > 0) {
      suggestions.push(
        `- 사용률이 낮은 기능들의 발견 가능성 향상: ${underusedFeatures
          .map(([name]) => name)
          .join(", ")}`
      );
    }
    ...
    if (this.userBehavior.averageSessionDuration < 300000) {
      // 5분 미만
      suggestions.push(
        "- 평균 세션 시간이 짧음 - 사용자 참여도 향상 방안 필요"
      );
    }

    // 자주 발생하는 오류
    const frequentErrors = Array.from(
      this.userBehavior.commonErrorTypes.entries()
    ).filter(([, count]) => count > 5);

    if (frequentErrors.length > 0) {
      suggestions.push(
        `- 자주 발생하는 오류 해결 우선: ${frequentErrors
          .map(([name]) => name)
          .join(", ")}`
      );
    }
  ...
  private getOrCreateUserId(): string {
    const config = vscode.workspace.getConfiguration("hapa");
    let userId = config.get("telemetryUserId") as string;
        ...
        JSON.stringify(
          {
            ...
            userBehavior: {
              ...this.userBehavior,
              featureUsage: Array.from(
                this.userBehavior.featureUsage.entries()
              ),
              commonErrorTypes: Array.from(
                this.userBehavior.commonErrorTypes.entries()
              ),
            },
          },
    ...
    try {
      ...
      if (fs.existsSync(metricsPath)) {
        ...
        if (data.usageMetrics) {
          ...
          this.usageMetrics.features = new Map(
        ...
        if (data.userBehavior) {
          this.userBehavior = {
            ...
            lastActiveDate: new Date(
              data.userBehavior.lastActiveDate || Date.now()
            ),
          };
        }
      }
  ...
  getStatistics() {
    return {
      ...
      totalEvents: Array.from(this.userBehavior.featureUsage.values()).reduce(
        (a, b) => a + b,
        0
      ),
